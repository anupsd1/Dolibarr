...
function print_left_eldy_menu($db,$menu_array_before,$menu_array_after,&$tabMenu,&$menu,$noout=0,$forcemainmenu='',$forceleftmenu='',$moredata=null)
{
	...
	if ($mainmenu)
	{
		...
		if ($mainmenu == 'products')
		{
			...
			{
				$langs->load("stocks");
                                //By Anup -Changed $langs->trans(Warehouses) to $langs->trans(Sites) on line1293 and changed $langs->trans(MenuNewWarehouses) to $langs->(New Sites) on line 1294
				$newmenu->add("/product/stock/index.php?leftmenu=stock", $langs->trans("Sites"), 0, $user->rights->stock->lire, '', $mainmenu, 'stock');
				$newmenu->add("/product/stock/card.php?action=create", $langs->trans("New Site"), 1, $user->rights->stock->creer);
