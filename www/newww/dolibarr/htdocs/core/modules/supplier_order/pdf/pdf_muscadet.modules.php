			...
			if (file_exists($dir))
			{
				...
				if (empty($this->atleastonediscount) && empty($conf->global->PRODUCT_USE_UNITS))
				{
					...
					$this->posxtva+=($this->postotalht - $this->posxdiscount);
                                        ...
                                        //BY ANUP
                                        $this->anup+=($this->postotalht - $this->posxdiscount);
				...
				if (! empty($object->note_public))
				{
					...
					$pdf->SetFont('','', $default_font_size - 1);
					$pdf->writeHTMLCell(190, 20, $this->posxdesc-1, $tab_top-1, dol_htmlentitiesbr($object->note_public, 1), 0, 1);
					$nexY = $pdf->GetY();
				...
				for ($i = 0 ; $i < $nblignes ; $i++)
				{
					...
					if (empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT))
					{
						//$vat_rate = pdf_getlinevatrate($object, $i, $outputlangs, $hidedetails);
                                                $myhsncode = pdf_gethsncode($object, $i, $outputlangs, $hidedetails);
						$pdf->SetXY($this->posxanup, $curY);
						$pdf->MultiCell($this->posxtva-$this->posxanup-1, 4, $myhsncode, 0, 'R');
					}
	...
	function _tableau_tot(&$pdf, $object, $deja_regle, $posy, $outputlangs)
	{
		...
		foreach( $this->tva as $tvakey => $tvaval )
		{
			if ($tvakey > 0)    // On affiche pas taux 0
			{
				...
				$totalvat =$outputlangs->transcountrynoentities("TotalGST",$mysoc->country_code).' ';
		...
		if (! $this->atleastoneratenotnull) // If no vat at all
		{
			$index++;
			$pdf->SetXY($col1x, $tab2_top + $tab2_hl * $index);
			$pdf->MultiCell($col2x-$col1x, $tab2_hl, $outputlangs->transcountrynoentities("TotalGST", $mysoc->country_code), 0, 'L', 1);
	...
	function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop=0, $hidebottom=0, $currency='')
	{
		...
		if (empty($hidetop))
		{
			...
			$pdf->MultiCell(108,2, $outputlangs->transnoentities("Designation"),'','L');
                ...
                //START BY ANUP
                $pdf->line($this->posxanup, $tab_top, $this->posxanup, $tab_top + $tab_height);
			if (empty($hidetop))
			{
    			$pdf->SetXY($this->posxanup-1, $tab_top+1);
    			$pdf->MultiCell($this->posxtva-$this->posxanup+3,2, $outputlangs->transnoentitiesnoconv("HSNCODE"),'','C');
			}
        ...
        if (empty($conf->global->MAIN_GENERATE_DOCUMENTS_WITHOUT_VAT))
        {
			...
			if (empty($hidetop))
			{
    			...
    			$pdf->MultiCell($this->posxup-$this->posxtva+3,2, $outputlangs->transnoentities("GST"),'','C');
                        //$pdf->MultiCell($this->posxanup-$this->posxtva+3,2, $outputlangs->transnoentities("GST"),'','C');
			}
        }
	...
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
		...
		if ($showaddress)
		{
			...
			$pdf->SetXY($posx,$posy-5);
                        ...
                        //Modified by Anup
			$pdf->MultiCell(66,5, $outputlangs->transnoentities("Billing Address").":", 0, 'L');
			$pdf->SetXY($posx,$posy);
