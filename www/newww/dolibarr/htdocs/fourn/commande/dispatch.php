...
//START -included by anup 
require_once DOL_DOCUMENT_ROOT.'/core/lib/functions2.lib.php';
//require_once DOL_DOCUMENT_ROOT.'/core/lib/files.lib.php';
require_once DOL_DOCUMENT_ROOT.'/core/class/doleditor.class.php';
//END- included by Anup
		...
		if (preg_match('/^product_([0-9]+)_([0-9]+)$/i', $key, $reg))
		{
			...
			if (GETPOST($qty) != 0) {
				...
				if (! $error) {
                                        ...
                                        //START- by anup 
                                        global $langs, $myid, $conf;
                                        $myent = GETPOST($ent, 'int');
                                        $warehousename = getwarehouse($myent);
                                        //$warehousename = $warehouseobj->fetch($myent);
                                        $object->note_public = "<b>Shipping Address</b> : ".$warehousename;
                                        
                                        $ret = $object->fetch($myid);
                                        $model=$object->model_pdf;
                                        $outputlangs = $langs;
                                        //$outputlangs =new Translate('', $conf);
                                        $outputlangs->loadLangs(array("main", "orders", "companies", "bills", "dict", "products"));
                                        $result2=$object->generateDocument($model, $outputlangs, $hidedetails=0, $hidedesc=0, $hideref=0);
                                        //END - By Anup
...
if ($id > 0 || ! empty($ref)) {
  ...
  $reshook=$hookmanager->executeHooks('formObjectOptions',$parameters,$object,$action); // Note that $action and $object may have been modified by hook
	...
	if ($object->statut == CommandeFournisseur::STATUS_ORDERSENT
		...
		$listwarehouses = $entrepot->list_array(1);
                ...
                <script type="text/javascript">
                 $(document).ready(function(){
                    ...
                    var numrows=$("#mynumrows").val();
                    var warehouse = $("#entrepot_0_0 option:selected").val();
                    //$("#entrepot_0_0 option:selected").css("hidden", true);
                        
                    
                    hideallselect(numrows);
                    initallselect(warehouse, numrows);
                    
                    
                    $("#entrepot_0_0").change(function(){
                        var warehouse = $("#entrepot_0_0 option:selected").val();
                        //alert(warehouse);
                        $.ajax({
                            url: "",
                    ...
                    $(".myform").submit(function(){
                        var i = 1;
                        while(i<numrows){
                            var toselect = "#entrepot_0_"+i;

                            var ans=$(toselect).prop("disabled", false);
                            console.log(ans)
                        ...
                        var j=0;
                        while(j<numrows){
                            var qtydispatched = "#qty_0_"+j;
                            $(qtydispatched).prop("disabled", false);
                            j++;
                        }
                    })
                ...
                function hideallselect(numrows){
                    var i = 1;
                    while(i<numrows){
                        var toselect = "#entrepot_0_"+i;
                        ...
                        //$(toselect).attr("style", "display:none");
                        //$(toselect).hide();
                        //var display = $(toselect).css("display");
                        //alert(display);
                        i++;
                    }
                    ...
                    while(j<numrows){
                        var qtydispatched = "#qty_0_"+j;
                        $(qtydispatched).prop("disabled", true);
                        j++;
                    }
                }
                
                function initallselect(warehouse, numrows){
                    var i = 1;
                    while(i<numrows){
                        var toselect = "#entrepot_0_"+i;
                        $(toselect).val(warehouse);
                        var item = $(toselect).val();
                        console.log(item);
                        i++;
                    }
                }
		...
		print '<form method="POST" action="dispatch.php?id=' . $object->id . '" class="myform">';
		...
		if ($resql) {
			...
			$nbproduct = 0;			// Nb of predefined product lines to dispatch (already done or not) if SUPPLIER_ORDER_DISABLE_STOCK_DISPATCH_WHEN_TOTAL_REACHED is off (default)
                        ...
                        print '<input type="hidden"  id="mynumrows" value="'.$num.'">';
			while ( $i < $num ) {
				...
				if (! $objp->fk_product > 0) {
					...
					if ($remaintodispatch || empty($conf->global->SUPPLIER_ORDER_DISABLE_STOCK_DISPATCH_WHEN_TOTAL_REACHED)) {
						...
						if (count($listwarehouses) > 1) {
							print $formproduct->selectWarehouses('', "entrepot" . $suffix, '', 1, 0, $objp->fk_product, '', 1, 0, null, 'csswarehouse'.$suffix);
...
function getwarehouse($warid)
{
//    require_once DOL_DOCUMENT_ROOT.'/product/stock/class/entrepot.class.php';
//    $warehouseobj = new Entrepot($db);
    global $db;
    $sql2= "SELECT rowid, fk_parent, ref as label, description, statut, lieu, address, zip, town, fk_pays as country_id";
    $sql2.= " FROM ".MAIN_DB_PREFIX."entrepot";
		
    $sql2.= " WHERE rowid = '".$warid."'";
    $result3 = $db->query($sql2);
    $newobj=$db->fetch_object($result3);
    if($newobj->address)
        return $newobj->label." - ". $newobj->address;
    else 
        return $newobj->label." - No address mentioned";
