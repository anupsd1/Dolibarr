...
/**
 *   	\file       cameraandxray_card.php
 *		\ingroup    miscellaneous
 *		\brief      Page to create/edit/view cameraandxray
 */
...
dol_include_once('/miscellaneous/lib/cameraandxray.lib.php');
...
$object=new CameraAndXRAY($db);
$extrafields = new ExtraFields($db);
$diroutputmassaction=$conf->miscellaneous->dir_output . '/temp/massgeneration/'.$user->id;
$hookmanager->initHooks(array('cameraandxraycard'));     // Note that conf->hooks_modules contains array
// Fetch optionals attributes and labels
$extralabels = $extrafields->fetch_name_optionals_label('cameraandxray');
...
if (empty($reshook))
{
	...
	$backurlforlist = dol_buildpath('/miscellaneous/cameraandxray_list.php',1);
	...
	$trackid='cameraandxray'.$object->id;
...
if ($action == 'create')
{
	print load_fiche_titre($langs->trans("NewObject", $langs->transnoentitiesnoconv("CameraAndXRAY")));
...
{
	print load_fiche_titre($langs->trans("CameraAndXRAY"));
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create')))
{
	...
	$head = cameraandxrayPrepareHead($object);
	dol_fiche_head($head, 'card', $langs->trans("CameraAndXRAY"), -1, 'cameraandxray@miscellaneous');
	...
	if ($action == 'delete')
	{
	    $formconfirm = $form->formconfirm($_SERVER["PHP_SELF"] . '?id=' . $object->id, $langs->trans('DeleteCameraAndXRAY'), $langs->trans('ConfirmDeleteCameraAndXRAY'), 'confirm_delete', '', 0, 1);
	}
	...
	// Object card
	// ------------------------------------------------------------
	$linkback = '<a href="' .dol_buildpath('/miscellaneous/cameraandxray_list.php',1) . '?restore_lastsearch_values=1' . (! empty($socid) ? '&socid=' . $socid : '') . '">' . $langs->trans("BackToList") . '</a>';
	...
	*/
	...
	if ($action != 'presend')
	{
		...
		*/
	    ...
	    $linktoelem = $form->showLinkToObjectBlock($object, null, array('cameraandxray'));
	    ...
	    $morehtmlright = '<a href="'.dol_buildpath('/miscellaneous/cameraandxray_info.php', 1).'?id='.$object->id.'">';
	    ...
	    $somethingshown = $formactions->showactions($object, 'cameraandxray', $socid, 1, '', $MAXEVENT, '', $morehtmlright);
