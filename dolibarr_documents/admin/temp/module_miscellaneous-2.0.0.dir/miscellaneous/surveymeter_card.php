...
/**
 *   	\file       surveymeter_card.php
 *		\ingroup    miscellaneous
 *		\brief      Page to create/edit/view surveymeter
 */
...
dol_include_once('/miscellaneous/class/surveymeter.class.php');
dol_include_once('/miscellaneous/lib/surveymeter.lib.php');
...
$object=new SurveyMeter($db);
$extrafields = new ExtraFields($db);
$diroutputmassaction=$conf->miscellaneous->dir_output . '/temp/massgeneration/'.$user->id;
$hookmanager->initHooks(array('surveymetercard'));     // Note that conf->hooks_modules contains array
// Fetch optionals attributes and labels
$extralabels = $extrafields->fetch_name_optionals_label('surveymeter');
...
if (empty($reshook))
{
	...
	$backurlforlist = dol_buildpath('/miscellaneous/surveymeter_list.php',1);
...
if ($action == 'create')
{
	print load_fiche_titre($langs->trans("NewObject", $langs->transnoentitiesnoconv("SurveyMeter")));
...
{
	print load_fiche_titre($langs->trans("SurveyMeter"));
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create')))
{
	...
	$head = surveymeterPrepareHead($object);
	dol_fiche_head($head, 'card', $langs->trans("SurveyMeter"), -1, 'surveymeter@miscellaneous');
	...
	if ($action == 'delete')
	{
	    $formconfirm = $form->formconfirm($_SERVER["PHP_SELF"] . '?id=' . $object->id, $langs->trans('DeleteSurveyMeter'), $langs->trans('ConfirmDeleteSurveyMeter'), 'confirm_delete', '', 0, 1);
	}
	...
	// Object card
	// ------------------------------------------------------------
	$linkback = '<a href="' .dol_buildpath('/miscellaneous/surveymeter_list.php',1) . '?restore_lastsearch_values=1' . (! empty($socid) ? '&socid=' . $socid : '') . '">' . $langs->trans("BackToList") . '</a>';
	...
	*/
	...
	if ($action != 'presend')
	{
		...
		*/
	    ...
	    $linktoelem = $form->showLinkToObjectBlock($object, null, array('surveymeter'));
	    ...
	    $morehtmlright = '<a href="'.dol_buildpath('/miscellaneous/surveymeter_info.php', 1).'?id='.$object->id.'">';
	    ...
	    $somethingshown = $formactions->showactions($object, 'surveymeter', $socid, 1, '', $MAXEVENT, '', $morehtmlright);
