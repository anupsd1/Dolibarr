	...
	function selectForFormsList($objecttmp, $htmlname, $preselectedvalue, $showempty='', $myvar='', $searchkey='', $placeholder='', $morecss='', $moreparams='', $forcecombo=0, $outputmode=0)
	{
		...
		// Search data
            ...
            if(($objecttmp->element == 'surveymeter') || ($objecttmp->element == 'dosimeter') || ($objecttmp->element == 'gammaareazonemonitor') || ($objecttmp->element == 'densitometer'))
            {
                ...
                {
                    $sql = "SELECT t.rowid, ".$fieldstoshow.", t.elid, t.eloraval FROM ".MAIN_DB_PREFIX .$objecttmp->table_element." as t";
                }
                else
                {
			...
			if ($num)
			{
					...
					if (empty($outputmode))
					{
						if ($preselectedvalue > 0 && $preselectedvalue == $obj->rowid)
						{
                                                        if($obj->elid)
                                                        {
                                                            $out.= '<option value="'.$obj->rowid.'" selected>'.$obj->elid."-".$obj->eloraval."-".$label."-".'</option>';
						...
						else
						{
                                                        if($obj->elid)
                                                        {
                                                            ...
                                                            $out.= '<option value="'.$obj->rowid.'">'.$obj->elid."-".$label."-".$obj->eloraval.'</option>';
                                                        }
                                                        else
                                                        {
