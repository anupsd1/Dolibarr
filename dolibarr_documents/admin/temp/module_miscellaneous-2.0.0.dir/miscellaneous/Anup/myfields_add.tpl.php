...
if($object->element == 'safetyeqcheck')
{
    foreach($object->fields as $key => $val)
    {
        if($val['label'] == 'Radiation Survey Meter 1' || $val['label']== 'Pocket Dosimeter 1')
        {
            //print '<br>'.$langs->trans($val['label']);
            print '<tr id="field_'.$key.'">';
            print '<td';
            print ' class="titlefieldcreate';
            if ($val['notnull'] > 0) print ' fieldrequired';
            ...
            print '<div class="anup">';
            print '<table border="1"><thead><th>Elora id-Model-Calibration validity</th><th>Working?(Yes/No)</th></thead>';
            print '<tr>';
            print '<td>';
            if (in_array($val['type'], array('int', 'integer'))) $value = GETPOST($key, 'int');
            elseif ($val['type'] == 'text' || $val['type'] == 'html') $value = GETPOST($key, 'none');
        ...
        elseif (($val['label'] == 'RSMWORK1') || ($val['label'] == 'RSMWORK2') || ($val['label']=='DOSWORK1') || ($val['label']=='DOSWORK2'))
        {
            print '<td>';
            if (in_array($val['type'], array('int', 'integer'))) $value = GETPOST($key, 'int');
            elseif ($val['type'] == 'text' || $val['type'] == 'html') $value = GETPOST($key, 'none');
        ...
        elseif(($val['label'] == 'Radiation Survey Meter 2') || ($val['label'] == 'Radiation Survey Meter 3') || ($val['label'] == 'Pocket Dosimeter 2') || ($val['label']=='Pocket Dosimeter 3'))
        {
            print '<tr>';
            print '<td>';
            if (in_array($val['type'], array('int', 'integer'))) $value = GETPOST($key, 'int');
        ...
        elseif(($val['label'] == 'RSMWORK3') || $val['label'] == 'DOSWORK3') 
        {
            //print '<tr>';
            print '<td>';
            if (in_array($val['type'], array('int', 'integer'))) $value = GETPOST($key, 'int');
        ...
        {
            //print '</table>';
            //print '<table class="border centerpercent">';
            //print '<tbody>';
...
elseif($object->element == 'advancesalary')
{
    foreach ($object->fields as $key => $val)
    {
        if($val['label'] != "Salary")
        ...
        {
            print '&emsp; <tr><td>'; print '</td><td>';
            print '<p>To:<br>Head HR<br>Injotech<br>';
            print 'I <input type="text" id="employeename" readonly> would like to have an advance against salary of Rs.';
            //print '</p></td><td>';
            if (in_array($val['type'], array('int', 'integer'))) $value = GETPOST($key, 'int');
            elseif ($val['type'] == 'text' || $val['type'] == 'html') $value = GETPOST($key, 'none');
            else $value = GETPOST($key, 'alpha');
            print $object->showInputField($val, $key, $value, '', '', '', 0);
            print '</p></td></tr>';
            print '<tr><td>I hereby confirm that I had submitted the following original documents as a securitiy against the same:</td></tr>';
        }
    }
}
elseif($object->element != 'dailywork')
{
