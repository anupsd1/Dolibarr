...
/**
 *   	\file       densitometer_card.php
 *		\ingroup    miscellaneous
 *		\brief      Page to create/edit/view densitometer
 */
...
dol_include_once('/miscellaneous/class/densitometer.class.php');
dol_include_once('/miscellaneous/lib/densitometer.lib.php');
...
$object=new Densitometer($db);
$extrafields = new ExtraFields($db);
$diroutputmassaction=$conf->miscellaneous->dir_output . '/temp/massgeneration/'.$user->id;
$hookmanager->initHooks(array('densitometercard'));     // Note that conf->hooks_modules contains array
// Fetch optionals attributes and labels
$extralabels = $extrafields->fetch_name_optionals_label('densitometer');
...
if (empty($reshook))
{
	...
	$backurlforlist = dol_buildpath('/miscellaneous/densitometer_list.php',1);
	...
	$trackid='densitometer'.$object->id;
...
if ($action == 'create')
{
	print load_fiche_titre($langs->trans("NewObject", $langs->transnoentitiesnoconv("Densitometer")));
...
{
	print load_fiche_titre($langs->trans("Densitometer"));
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create')))
{
	...
	$head = densitometerPrepareHead($object);
	dol_fiche_head($head, 'card', $langs->trans("Densitometer"), -1, 'densitometer@miscellaneous');
	...
	if ($action == 'delete')
	{
	    $formconfirm = $form->formconfirm($_SERVER["PHP_SELF"] . '?id=' . $object->id, $langs->trans('DeleteDensitometer'), $langs->trans('ConfirmDeleteDensitometer'), 'confirm_delete', '', 0, 1);
	}
	...
	// Object card
	// ------------------------------------------------------------
	$linkback = '<a href="' .dol_buildpath('/miscellaneous/densitometer_list.php',1) . '?restore_lastsearch_values=1' . (! empty($socid) ? '&socid=' . $socid : '') . '">' . $langs->trans("BackToList") . '</a>';
	...
	*/
	...
	if ($action != 'presend')
	{
		...
		*/
	    ...
	    $linktoelem = $form->showLinkToObjectBlock($object, null, array('densitometer'));
	    ...
	    $morehtmlright = '<a href="'.dol_buildpath('/miscellaneous/densitometer_info.php', 1).'?id='.$object->id.'">';
	    ...
	    $somethingshown = $formactions->showactions($object, 'densitometer', $socid, 1, '', $MAXEVENT, '', $morehtmlright);
