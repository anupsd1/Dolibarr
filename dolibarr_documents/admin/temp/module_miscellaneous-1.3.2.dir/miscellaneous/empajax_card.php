...
/**
 *   	\file       empajax_card.php
 *		\ingroup    miscellaneous
 *		\brief      Page to create/edit/view empajax
 */
...
dol_include_once('/miscellaneous/lib/empajax.lib.php');
...
$object=new EmpAjax($db);
$extrafields = new ExtraFields($db);
$diroutputmassaction=$conf->miscellaneous->dir_output . '/temp/massgeneration/'.$user->id;
$hookmanager->initHooks(array('empajaxcard'));     // Note that conf->hooks_modules contains array
// Fetch optionals attributes and labels
$extralabels = $extrafields->fetch_name_optionals_label('empajax');
...
//include_once 'myactions_fetchobject.inc.php';
...
if (empty($reshook))
{
	...
	$backurlforlist = dol_buildpath('/miscellaneous/empajax_list.php',1);
...
if ($action == 'create')
{
	print load_fiche_titre($langs->trans("NewObject", $langs->transnoentitiesnoconv("EmpAjax")));
...
{
	print load_fiche_titre($langs->trans("EmpAjax"));
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create')))
{
	...
	$head = empajaxPrepareHead($object);
	dol_fiche_head($head, 'card', $langs->trans("EmpAjax"), -1, 'empajax@miscellaneous');
	...
	if ($action == 'delete')
	{
	    $formconfirm = $form->formconfirm($_SERVER["PHP_SELF"] . '?id=' . $object->id, $langs->trans('DeleteEmpAjax'), $langs->trans('ConfirmDeleteEmpAjax'), 'confirm_delete', '', 0, 1);
	}
	...
	// Object card
	// ------------------------------------------------------------
	$linkback = '<a href="' .dol_buildpath('/miscellaneous/empajax_list.php',1) . '?restore_lastsearch_values=1' . (! empty($socid) ? '&socid=' . $socid : '') . '">' . $langs->trans("BackToList") . '</a>';
	...
	*/
	...
	if ($action != 'presend')
	{
		...
		*/
	    ...
	    $linktoelem = $form->showLinkToObjectBlock($object, null, array('empajax'));
	    ...
	    $morehtmlright = '<a href="'.dol_buildpath('/miscellaneous/empajax_info.php', 1).'?id='.$object->id.'">';
	    ...
	    $somethingshown = $formactions->showactions($object, 'empajax', $socid, 1, '', $MAXEVENT, '', $morehtmlright);
