...
/**
 *   	\file       dosimeter_card.php
 *		\ingroup    miscellaneous
 *		\brief      Page to create/edit/view dosimeter
 */
...
dol_include_once('/miscellaneous/class/dosimeter.class.php');
dol_include_once('/miscellaneous/lib/dosimeter.lib.php');
...
$object=new Dosimeter($db);
$extrafields = new ExtraFields($db);
$diroutputmassaction=$conf->miscellaneous->dir_output . '/temp/massgeneration/'.$user->id;
$hookmanager->initHooks(array('dosimetercard'));     // Note that conf->hooks_modules contains array
// Fetch optionals attributes and labels
$extralabels = $extrafields->fetch_name_optionals_label('dosimeter');
...
if (empty($reshook))
{
	...
	$backurlforlist = dol_buildpath('/miscellaneous/dosimeter_list.php',1);
...
if ($action == 'create')
{
	print load_fiche_titre($langs->trans("NewObject", $langs->transnoentitiesnoconv("Dosimeter")));
...
{
	print load_fiche_titre($langs->trans("Dosimeter"));
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create')))
{
	...
	$head = dosimeterPrepareHead($object);
	dol_fiche_head($head, 'card', $langs->trans("Dosimeter"), -1, 'dosimeter@miscellaneous');
	...
	if ($action == 'delete')
	{
	    $formconfirm = $form->formconfirm($_SERVER["PHP_SELF"] . '?id=' . $object->id, $langs->trans('DeleteDosimeter'), $langs->trans('ConfirmDeleteDosimeter'), 'confirm_delete', '', 0, 1);
	}
	...
	// Object card
	// ------------------------------------------------------------
	$linkback = '<a href="' .dol_buildpath('/miscellaneous/dosimeter_list.php',1) . '?restore_lastsearch_values=1' . (! empty($socid) ? '&socid=' . $socid : '') . '">' . $langs->trans("BackToList") . '</a>';
	...
	*/
	...
	if ($action != 'presend')
	{
		...
		*/
	    ...
	    $linktoelem = $form->showLinkToObjectBlock($object, null, array('dosimeter'));
	    ...
	    $morehtmlright = '<a href="'.dol_buildpath('/miscellaneous/dosimeter_info.php', 1).'?id='.$object->id.'">';
	    ...
	    $somethingshown = $formactions->showactions($object, 'dosimeter', $socid, 1, '', $MAXEVENT, '', $morehtmlright);
