...
abstract class MyActionsObject
{
        public $myvar;
	/**
		...
		elseif ($type == 'double')
		{
			if (!empty($value)) {		// $value in memory is a php numeric, we format it into user number format.
				$value=price($value);
                        ...
                        //else{
			//$out='<input type="text" class="flat '.$showsize.' maxwidthonsmartphone" name="'.$keyprefix.$key.$keysuffix.'" id="'.$keyprefix.$key.$keysuffix.'" value="'.$value.'" '.($moreparam?$moreparam:'').'> ';
                        if(($keyprefix.$key.$keysuffix == "ari") || ($keyprefix.$key.$keysuffix == "arii") || ($keyprefix.$key.$keysuffix == "ariii") || ($keyprefix.$key.$keysuffix == "ariv"))
                        {
	...
	function showOutputField($val, $key, $value, $moreparam='', $keysuffix='', $keyprefix='', $showsize=0)
	{
		...
		{
			//require_once DOL_DOCUMENT_ROOT.'/core/class/html.form.class.php';
                        require_once 'Anup/myhtmlform.class.php';
			//$form=new Form($this->db);
                        $form = new CustomForm($this->db);
		}
	...
	public function createCommon(User $user, $notrigger = false)
	{
		...
		foreach($keys as $key)
		{
			// If field is an implicit foreign key field - The next three lines were written by ANUP!
			...
			// If field is an implicit foreign key field-FIRST 3 lines by ANUP
